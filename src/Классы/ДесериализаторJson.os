...
Функция ПреобразоватьОбъектДесериализации(ДесериализованныйОбъект, ТипОбъекта, ТипЭлементов = Неопределено)
	...
	ИначеЕсли ТипОбъекта = Тип("ТаблицаЗначений") Тогда
		Возврат ПреобразоватьТаблицуЗначений(ДесериализованныйОбъект, ТипОбъекта);
...
Функция ПреобразоватьТаблицуЗначений(Объект, ТипОбъекта)
	...
	Для Каждого ЗначенияКолонок Из Объект Цикл
		...
		Если Не (ТипЗнч(ЗначенияКолонок) = Тип("Структура") Или ТипЗнч(ЗначенияКолонок) = Тип("Соответствие")) Тогда
			Лог.Ошибка("Строка таблицы должна быть объектом, получен тип '%1'", ТипЗнч(ЗначенияКолонок));
			Продолжить;
		КонецЕсли;
		...
		Для Каждого КлючИЗначение Из ЗначенияКолонок Цикл
			...
			Если ДобавленныеКолонки[ИмяКолонки] = Неопределено Тогда
