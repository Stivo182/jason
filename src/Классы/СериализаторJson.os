...
//   ПараметрыЗаписи - ПараметрыЗаписиJSON, Неопределено - Оформление результата.
//                     Неопределено - перенос строк в стиле Unix.
// Возвращаемое значение:
//   Строка
//
Функция Сериализовать(Объект, ПараметрыЗаписи = Неопределено) Экспорт
	ОбъектСериализации = ПреобразоватьОбъектСериализации(Объект);

	Оформление = ?(ПараметрыЗаписи = Неопределено,
		Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Unix),
		ПараметрыЗаписи);

	Запись = Новый ЗаписьJSON;
	Запись.УстановитьСтроку(Оформление);
...
Функция ПреобразоватьОбъектСериализации(Объект)
	...
	ИначеЕсли ТипОбъекта = Тип("ТаблицаЗначений") Тогда
		Результат = ПреобразоватьТаблицуЗначений(Объект);
...
Функция ПреобразоватьТаблицуЗначений(ТаблицаЗначений) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		...
		Для Каждого Колонка Из ТаблицаЗначений.Колонки Цикл
			...
			ЗначенияКолонок.Вставить(Колонка.Имя, ПреобразованноеЗначение);
		КонецЦикла;
