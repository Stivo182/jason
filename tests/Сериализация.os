...
Процедура СериализацияКласса() Экспорт
	...
	ТестовыйКласс.Таблица = Новый ТаблицаЗначений();
	ТестовыйКласс.Таблица.Колонки.Добавить("колонка1");
	ТестовыйКласс.Таблица.Колонки.Добавить("колонка2");
	Стр1 = ТестовыйКласс.Таблица.Добавить();
	Стр1.Колонка1 = 1;
	Стр1.Колонка2 = 2;
	Стр2 = ТестовыйКласс.Таблица.Добавить();
	Стр2.Колонка1 = 3;
	Стр2.Колонка2 = 4;
	...
	Ожидаем.Что(ПрочитанныйОбъект.Таблица).ИмеетТип("Массив");
	Ожидаем.Что(ПрочитанныйОбъект.Таблица).ИмеетДлину(2);
	Ожидаем.Что(ПрочитанныйОбъект.Таблица[0]["колонка1"]).Равно(1);
	Ожидаем.Что(ПрочитанныйОбъект.Таблица[0]["колонка2"]).Равно(2);
	Ожидаем.Что(ПрочитанныйОбъект.Таблица[1]["колонка1"]).Равно(3);
	Ожидаем.Что(ПрочитанныйОбъект.Таблица[1]["колонка2"]).Равно(4);
...
&Тест
Процедура СериализацияКорневойТаблицыЗначений() Экспорт

	// Дано
	Таблица = Новый ТаблицаЗначений();
	Таблица.Колонки.Добавить("x");
	Таблица.Колонки.Добавить("y");

	Стр1 = Таблица.Добавить();
	Стр1.x = 1;
	Стр1.y = 2;

	Стр2 = Таблица.Добавить();
	Стр2.x = "a";
	Стр2.y = Новый Структура("x", 3);

	Сериализатор = Новый СериализаторJson();

	// Когда
	JSON = Сериализатор.Сериализовать(Таблица);
	Объект = ПрочитатьИзJSON(JSON);

	// Тогда
	// JSON-объект читается как Массив
	Ожидаем.Что(Объект).ИмеетТип("Массив");
	Ожидаем.Что(Объект).ИмеетДлину(2);
	Ожидаем.Что(Объект[0]["x"]).Равно(1);
	Ожидаем.Что(Объект[0]["y"]).Равно(2);
	Ожидаем.Что(Объект[1]["x"]).Равно("a");
	Ожидаем.Что(Объект[1]["y"]).ИмеетТип("Структура");
	Ожидаем.Что(Объект[1]["y"]["x"]).Равно(3);

КонецПроцедуры

&Тест
Процедура СериализацияПустойТаблицыЗначений() Экспорт
	...
	// Когда
	JSON = Сериализатор.Сериализовать(Таблица);
	ПрочитаннаяТаблица = Десериализатор.Десериализовать(JSON, Тип("ТаблицаЗначений"));

	// Тогда
	Ожидаем.Что(JSON).Равно("[]");
	Ожидаем.Что(ПрочитаннаяТаблица).ИмеетТип("ТаблицаЗначений");
	Ожидаем.Что(ПрочитаннаяТаблица).ИмеетДлину(0);
	Ожидаем.Что(ПрочитаннаяТаблица.Колонки).ИмеетДлину(0);
...
// Оформление задаёт потребитель: одному нужен читаемый JSON, другому - компактный,
// и выбирать за них сериализатор не должен.
&Тест
Процедура СериализацияПринимаетПараметрыЗаписи() Экспорт
	...
	// Когда
	JSON = Сериализатор.Сериализовать(Стр, Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));

	// Тогда
	Ожидаем.Что(JSON, "компактная запись без переносов").Равно("{""a"":1,""b"":2}");

КонецПроцедуры

// Вызовы без параметра оформления должны работать как раньше.
&Тест
Процедура СериализацияБезПараметровЗаписиОставляетПрежнееОформление() Экспорт
	...
	// Когда
	ПоУмолчанию = Сериализатор.Сериализовать(Стр);
	ЯвноеОформление = Сериализатор.Сериализовать(Стр, Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Unix));

	// Тогда
	Ожидаем.Что(ПоУмолчанию, "умолчание не изменилось").Равно(ЯвноеОформление);
	Ожидаем.Что(СтрНайти(ПоУмолчанию, Символы.ПС), "перенос строки на месте").Больше(0);
