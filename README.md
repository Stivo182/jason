...
- Коллекции: Массив, Структура, Соответствие, ТаблицаЗначений (вложенные структуры поддерживаются)
...
- Коллекции: Массив, Структура, Соответствие, ТаблицаЗначений
- Пользовательские классы (через рефлексию и аннотации)

Особенности десериализации:
- JSON null читается как Неопределено.
- ТаблицаЗначений представляется массивом JSON-объектов. Типы колонок не сохраняются: при десериализации колонки создаются без описания типов. У пустой таблицы также не сохраняются имена колонок, поскольку она представляется пустым массивом `[]`.
